  ...
  const processExcelFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      for (const sheetName of workbook.SheetNames) {
        if (totalRowsProcessed >= MAX_ROWS) break;
        
        const worksheet = workbook.Sheets[sheetName];
        // Convert sheet to array of arrays for easy processing
        ...
        for (let rowNum = 0; rowNum < Math.min(5, rows.length); rowNum++) {
          const row = rows[rowNum];
          for (let colNum = 0; colNum < row.length; colNum++) {
            const cellValue = getCellStringValue(row[colNum]);
            if (!cellValue) continue;
            const normalized = cellValue.toLowerCase().trim();
            if (containerColumnIndex === null && isContainerColumnHeader(cellValue)) {
              containerColumnIndex = colNum;
              headerRowIndex = rowNum;
            }
            if (consigneeColumnIndex === null && ['consignee', 'consignee name', 'customer', 'client', 'receiver', 'importer'].some((name) => normalized === name || normalized.includes(name))) {
              consigneeColumnIndex = colNum;
              headerRowIndex = rowNum;
            }
            if (arrivalColumnIndex === null && ['arrival', 'arrival date', 'eta', 'estimated arrival', 'arrival_date', 'date of arrival', 'ata'].some((name) => normalized === name || normalized.includes(name))) {
              arrivalColumnIndex = colNum;
              headerRowIndex = rowNum;
            }
          }
        ...
        if (containerColumnIndex !== null) {
          for (let rowNum = headerRowIndex + 1; rowNum < rows.length; rowNum++) {
            if (totalRowsProcessed >= MAX_ROWS || shipmentMap.size >= MAX_CONTAINERS) break;
            totalRowsProcessed++;
            const row = rows[rowNum];
            const stringValue = getCellStringValue(row[containerColumnIndex]);
            if (!stringValue) continue;
            const containerNumber = cleanContainerNumber(stringValue);
            if (containerNumber && !shipmentMap.has(containerNumber)) {
              const consignee = consigneeColumnIndex !== null ? getCellStringValue(row[consigneeColumnIndex])?.trim() || null : null;
              let arrivalDate: string | null = null;
              if (arrivalColumnIndex !== null) {
                const rawVal = row[arrivalColumnIndex];
                if (rawVal instanceof Date && !isNaN(rawVal.getTime())) {
                  arrivalDate = rawVal.toISOString().slice(0, 10);
                } else {
                  const arrivalRaw = getCellStringValue(rawVal)?.trim() || '';
                  if (arrivalRaw) {
                    const parsed = new Date(arrivalRaw);
                    arrivalDate = !isNaN(parsed.getTime()) ? parsed.toISOString().slice(0, 10) : arrivalRaw;
                  }
                }
              }
              shipmentMap.set(containerNumber, { containerNumber, consignee, arrivalDate });
            }
          }
        } else {
          // No header found — scan all cells for container numbers
          for (let rowNum = 0; rowNum < rows.length; rowNum++) {
            if (totalRowsProcessed >= MAX_ROWS || shipmentMap.size >= MAX_CONTAINERS) break;
            totalRowsProcessed++;
            const row = rows[rowNum];
            for (const cell of row) {
              if (shipmentMap.size >= MAX_CONTAINERS) break;
              const stringValue = getCellStringValue(cell);
              if (!stringValue) continue;
              const containerNumber = cleanContainerNumber(stringValue);
              if (containerNumber && !shipmentMap.has(containerNumber)) {
                shipmentMap.set(containerNumber, { containerNumber, consignee: null, arrivalDate: null });
              }
            }
          }
        }
      ...
      setError('Failed to read the Excel file. Please make sure it is a valid .xlsx or .xls file.');
    } finally {
      setIsReading(false);
    }
  }, [onFileProcessed]);
